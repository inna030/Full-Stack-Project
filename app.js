//avoid submitting code to the backend
...
//Change the color once you've click the chosen option.
let allSelects = document.querySelectorAll("select");
allSelects.forEach((select) => {
  select.addEventListener("change", (e) => {
    setGPA();
    changeColor(e.target); //e.target refers <select> tag
  });
});
...
function handleSorting(direction) {
  ...
  //after obtained object array, switch GPA from String to Integer
  ...
  //based on the content of the object array, updates webpage
  ...
  //update the grade for each sorted object
  graders = document.querySelectorAll("div.grader");
  for (let i = 0; i < graders.length; i++) {
    graders[i].children[3].value = objectArray[i].class_grade;
  }

  // select event changes
